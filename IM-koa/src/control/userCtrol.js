...
userCtrol.post("/hasUser", async (ctx, next) => {
    ...
    let respond = await hasUserServ(data.userId, data.password);
...
userCtrol.post('/register', async ctx=>{
    ...
    let result = await addUserServ(data.userId, data.password)
...
userCtrol.get('/getProfile', async ctx => {
    ...
    let result;
    if(paramsObj.userId) {
       result = await getProfileServ(paramsObj.userId);
    }else {
